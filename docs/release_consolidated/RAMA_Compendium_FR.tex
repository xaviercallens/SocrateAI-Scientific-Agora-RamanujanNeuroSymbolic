...
\title{\Huge\textbf{Le Compendium RAMA}\\[0.5em] \large\textit{De la Géométrie de Ramanujan à la Physique Holographique et aux Fluides DualScale}}
...
\tableofcontents
\mainmatter
\chapter{Fondation Algébrique et Dualité-T}

Dans la tradition des travaux d'Andrews et Berndt sur le Cahier Perdu, nous commençons par formaliser la structure algébrique des quotients $\eta$ de Dedekind et leur rôle dans les théories de cordes.

\begin{definition}[Quotients $\eta$ et Séries $q$]
Un quotient $\eta$ de Dedekind généralisé est défini sur le disque unité $|q| < 1$ (avec $q = e^{2\pi i \tau}$, $\tau \in \mathbb{H}$) par la formule :
\[ f(\tau) = q^{p} \prod_{d|N} \eta(d\tau)^{r_d} = q^{p} \prod_{d|N} \left( q^{d/24} \prod_{n=1}^{\infty} (1 - q^{nd}) \right)^{r_d} \]
\end{definition}

\begin{theorem}[Invariants de Poids Modulaire et Charge Centrale]
Pour tout quotient $\eta$ défini par sa séquence de facteurs $(d, r_d)$, les invariants de symétrie conforme sont donnés exactement par :
\[ k = \frac{1}{2}\sum_{d|N} r_d \quad \text{et} \quad c_{\text{eff}} = \sum_{d|N} \frac{r_d}{d} \]
\end{theorem}

\begin{theorem}[Dualité-T et Transformation Modulaire Involutive]
Sous l'inversion modulaire $\tau \mapsto -1/\tau$ du groupe $SL(2, \mathbb{Z})$, le comportement asymptotique du quotient $\eta$ échange le régime ultra-violet (UV) et le régime infra-rouge (IR). Cette inversion correspond exactement à la symétrie de Dualité-T des cordes ($R \leftrightarrow \alpha'/R$), préservant le spectre microscopique.
\end{theorem}

\chapter{Asymptotiques Holographiques et Entropie BPS (HoloAlg)}

La méthode du cercle développée par Hardy et Ramanujan en 1918 (puis raffinée par Rademacher et Zuckerman) fournit l'outil analytique reliant les formes modulaires au comptage d'états dans la gravité quantique.

\begin{lemma}[Croissance Asymptotique de Rademacher]
Les coefficients de Fourier $a(n)$ d'un quotient $\eta$ ayant un poids modulaire $k=1/2$ et une charge centrale effective $c_{\text{eff}} > 0$ satisfont l'asymptotique stricte de Hardy-Ramanujan-Rademacher :
\[ a(n) \sim \frac{2\pi}{n^{k/2 + 1/4}} I_{k+1/2}\left( 2\pi \sqrt{\frac{c_{\text{eff}} \cdot n}{6}} \right) \]
\end{lemma}

\begin{theorem}[Comptage d'États BPS et Formule de Bekenstein-Hawking]
Pour la sous-classe des découvertes de Ramanujan satisfaisant la préservation de la supersymétrie ($k=1/2$), l'entropie microscopique log-asymptotique s'identifie exactement à l'entropie macroscopique du trou noir BPS holographique :
\[ S_{\text{BPS}} = \ln a(n) \approx 2\pi \sqrt{\frac{c_{\text{eff}} \cdot n}{6}} = \frac{\text{Aire}}{4G} \]
\end{theorem}

\begin{center}\includegraphics[width=0.75\textwidth]{../../figures/susy_distribution.png}\end{center}

\chapter{Correspondance Échelle-Duale et Régularité des Fluides}

Ce chapitre établit le résultat le plus marquant du compendium RAMA : l'application de la théorie des nombres de Ramanujan à la résolution de la compacité de Navier-Stokes via la dualité fluide-gravité.

\begin{axiom}[Dictionnaire Fluide-Gravité Holographique]
Le champ de vitesse fluide $v(x,t)$ sur le bord conforme d'un espace Anti-de Sitter (AdS) est généré directement par la superposition des modes spectraux de Fourier $a(n)$ dérivés du quotient $\eta$ de la théorie conforme en volume.
\end{axiom}

\begin{theorem}[Borne d'Enstrophie DualScale]
En vertu du Théorème 2.2, les coefficients de Fourier $a(n)$ sont exponentiellement décroissants aux hautes fréquences UV. Par conséquent, l'enstrophie intégrale du fluide frontière est strictement bornée par l'entropie BPS :
\[ \mathcal{E} = \int_{\Omega} |\nabla \times v|^2 \, dV \le \kappa \cdot S_{\text{BPS}} < \infty \]
\end{theorem}

\begin{theorem}[Convergence et Régularité d'Aubin-Lions]
Puisque l'enstrophie est uniformément bornée dans $L^2(0,T; H^1)$, le lemme de compacité d'Aubin-Lions garantit l'existence d'une limite forte compacte, prouvant que les équations fluides de Navier-Stokes ne développent aucune singularité en temps fini.
\end{theorem}

\chapter{Catalogue des Découvertes Formalisées}

Voici le catalogue des 50 premières découvertes novatrices (sur les 938 identifiées dans `namagiri.db`), traduites en notation standard de Dedekind et classées selon la référence Andrews-Berndt.

\begin{center}\includegraphics[width=0.75\textwidth]{../../figures/energy_landscape.png}\end{center}

\subsection*{Théorème RAMA ID: e46d9956-a9e}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{10: 2}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.167082
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: d08c97f0-c08}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(12/24) * \prod \eta(q^d)^{11: -4}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.348257
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 116e8203-b81}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{3: 3}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.220699
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 5312c21c-e52}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{6: 2}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.229479
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 8317b09b-72e}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{6: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.230124
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: eb03f21e-c0a}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{3: 4}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.452385
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: e10679e2-442}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(-2/24) * \prod \eta(q^d)^{10: 3}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.398060
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: f0d9021e-053}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(4/24) * \prod \eta(q^d)^{10: 3}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.398060
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: a626695f-1b1}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{4: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.194765
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: ade3259b-76b}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(-2/24) * \prod \eta(q^d)^{11: -3}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.365359
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 900bb087-3e5}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{11: -4}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.098257
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 579d5934-bc6}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{11: -3}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.115359
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: c222ac5c-515}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{8: 2}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.210918
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 5c01cd04-5f8}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(12/24) * \prod \eta(q^d)^{10: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.448926
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 2222e5b4-b6d}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(-12/24) * \prod \eta(q^d)^{11: -1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.438026
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 522b77ed-d01}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{9: -1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.209864
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 6c45da89-243}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{6: 3}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.241655
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 2c70cfff-714}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{8: 4}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.229529
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 930fe7cb-01c}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{10: 4}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.141857
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 807208fe-d33}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(1/24) * \prod \eta(q^d)^{4: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.444765
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 42763ef0-03b}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(-12/24) * \prod \eta(q^d)^{3: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.494506
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 89968a81-e4c}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{10: -2}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.474775
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 50a39513-894}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{8: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.220844
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: bbb9ebf8-2f1}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{9: 0}$
  \item \textbf{Niveau d'Énergie RAMA:} 0.993590
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 56f1cf6b-bfd}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{8: 3}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.213813
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 750e49ec-8cf}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{10: 3}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.148060
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 69ad9c99-0ec}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{5: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.222027
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 7859bd56-029}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(12/24) * \prod \eta(q^d)^{6: 2}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.479479
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: b4a25fbe-b90}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{1: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.216128
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: e56fdf39-8e3}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(-24/24) * \prod \eta(q^d)^{10: -1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.602772
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 30466116-cce}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(-12/24) * \prod \eta(q^d)^{8: 2}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.460918
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: e64fad96-dbd}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{9: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.338069
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 1ec02312-96d}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{3: -1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.218865
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: f2841284-167}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(4/24) * \prod \eta(q^d)^{10: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.448926
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 801f5539-45c}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{12: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.273315
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: f59ff17a-3f9}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(-2/24) * \prod \eta(q^d)^{8: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.470844
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 50156515-4e0}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(2/24) * \prod \eta(q^d)^{9: -1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.459864
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 41b228df-f54}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(4/24) * \prod \eta(q^d)^{8: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.470844
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: f963b410-c6d}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{10: 1}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.198926
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: b8d896c2-e08}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{12: -4}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.362490
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: ddcbfe4d-c08}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(-2/24) * \prod \eta(q^d)^{8: 3}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.463813
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: fe5e3852-880}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{10: 7}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.200173
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: ec2c7e82-933}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(-2/24) * \prod \eta(q^d)^{11: -4}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.348257
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: f2ec2a4c-70e}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(3/24) * \prod \eta(q^d)^{6: 4}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.516651
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 12fc5cc3-898}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(2/24) * \prod \eta(q^d)^{11: -3}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.365359
...
\subsection*{Théorème RAMA ID: f85e401e-a54}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{12: 3}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.332765
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: efeb392e-85d}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(-2/24) * \prod \eta(q^d)^{9: -2}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.438959
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: a9dd1cb0-200}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(-24/24) * \prod \eta(q^d)^{9: -4}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.435610
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: ae4ef401-c35}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(0/24) * \prod \eta(q^d)^{7: -3}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.246889
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}

\subsection*{Théorème RAMA ID: 08f66605-55a}
\begin{itemize}
  \item \textbf{Archétype Topologique:} Mock Theta Function
  \item \textbf{Formulation Mathématique:} $q^(12/24) * \prod \eta(q^d)^{3: 2}$
  \item \textbf{Niveau d'Énergie RAMA:} 1.437731
  \item \textbf{Correspondance Historique:} Andrews-Berndt Part I (2005)
\end{itemize}
...
\toprule
\textbf{ID} & \textbf{Archétype Topologique} & \textbf{Énergie RAMA} & \textbf{Réf. Andrews-Berndt} \\
\midrule
\endhead
