...
Voici un échantillon classifié de vecteurs d'état novateurs $| \Omega_{\mathbf{r}} angle$ identifiés par l'agent formel, regroupés selon la valeur propre de l'opérateur de Supersymétrie $\hat{\mathcal{W}}$.

\begin{center}\includegraphics[width=0.75\textwidth]{../../figures/energy_landscape.png}\end{center}

\section{Classe I : États BPS Protégés ($\hat{\mathcal{W}} = 1/2$)}

\section{Classe II : Fonctions Modulaires ($\hat{\mathcal{W}} = 0$)}
...
\section{Classe III : Vides Exotiques Briseurs de SUSY ($\hat{\mathcal{W}} \neq 1/2, 0$)}
